    ...
    def run(self):
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                for cask in casks:
                    updates.append({
                        'name': cask['name'][0] if isinstance(cask['name'], list) else cask['name'],
                        'id': cask['name'][0] if isinstance(cask['name'], list) else cask['token'], # token je ID
                        'current_ver': cask['installed_versions'][0] if cask['installed_versions'] else "?",
                        'new_ver': cask['current_version']
                    })
...
class UpdateWorker(QThread):
    ...
    def run(self):
        if self.update_all:
            self.log_signal.emit("--- AKTUALIZUJI VŠE ---\n")
            cmd = ['brew', 'upgrade', '--cask']
        else:
            self.log_signal.emit(f"--- AKTUALIZUJI: {self.app_id} ---\n")
            cmd = ['brew', 'upgrade', '--cask', self.app_id]
