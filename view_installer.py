...
class SearchWorker(QThread):
    ...
    def run(self):
        ...
        for term in search_terms:
            # FALLBACK: BREW SEARCH
            try:
                # brew search vrací jen text, brew info --json potřebuje přesné jméno
                # Pro jednoduchost parsujeme textový výstup
                cmd = ['brew', 'search', self.query]
                result = subprocess.run(cmd, capture_output=True, text=True)
                
                data = []
                if result.returncode == 0:
                    # Brew search rozděluje sekce na "Formulae" a "Casks"
                    lines = result.stdout.split('\n')
                    for line in lines:
                        clean = line.strip()
                        if not clean or "==>" in clean: continue
                        
                        # Brew search může vracet více položek na řádek? Někdy ano.
                        items = clean.split()
                        for item in items:
                            # Ignorujeme fonty pro přehlednost? (volitelné)
                            data.append({
                                "name": item.replace("-", " ").title(),
                                "id": item,
                                "version": "Latest", # Brew search nevrací verzi hned
                                "website": f"https://formulae.brew.sh/cask/{item}"
                            })
                
                self.finished.emit(data)

            except Exception as e:
                self.error.emit(f"Chyba hledání: {str(e)}")
