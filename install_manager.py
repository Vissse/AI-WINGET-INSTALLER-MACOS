...
class InstallationWorker(QThread):
    ...
    def run(self):
        ...
        # 1. Update Homebrew
        self.status_signal.emit("Aktualizace Homebrew...")
        self.log_signal.emit("--- BREW UPDATE ---\n")
        try:
            # Na macu nepoužíváme STARTUPINFO
            subprocess.run(['brew', 'update'], check=False)
            self.log_signal.emit(">>> Homebrew aktualizován.\n\n")
        except Exception as e:
            self.log_signal.emit(f"Warning: Update selhal ({str(e)}), pokračuji...\n\n")

        self.log_signal.emit("--- ZAHAJUJI INSTALACI ---\n")
        ...
        for i, app_data in enumerate(self.install_list):
            ...
            # Brew install příkaz
            # Většina GUI appek jsou "casks"
            cmd = ['brew', 'install', '--cask', app_id]
            
            # Pokud je v settings nastaveno force
            ...
            try:
                ...
                if process.returncode == 0:
                    ...
                    # Na macu se zástupci na plochu nedávají, jsou v Launchpadu (/Applications)
                else:
                    self.log_signal.emit(f"❌ Chyba při instalaci {app_name}.\n")
